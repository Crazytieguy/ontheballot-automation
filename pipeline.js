...
//        -> STANCE pass    ('decisive' classifier on each detected cell)
//        -> [optional] VERIFY triage  (off by default; necessity is TBD)
//        -> human is the final filter
//
// Run with:  Workflow({ scriptPath: "pipeline.js", args: { candidates: [...] } })
//   args.candidates: [{ id, name, state, party, seat, district? }]   (identity only;
//                     all evidence is fetched from the open web)
//   args.verify:     boolean (default false)
//
// Eval/scoring lives in eval/ (score.py, ensemble.py is the same union as below).
// Runner pitfalls (concurrency, retries, etc.): workflows/CLAUDE.md.
// ============================================================================

export const meta = {
  name: 'pipeline',
  description: 'Current best end-to-end: recall-first detect (3 diverse methods) -> ensemble union -> decisive stance -> optional verify triage',
  phases: [
    { title: 'Detect', detail: '3 diverse detectors per candidate, unioned' },
    { title: 'Stance', detail: "decisive stance on each detected cell" },
...
// ---------------------------------------------------------------------------
// SHARED PROMPT BLOCKS  (read these — this is exactly what the agents see)
// ---------------------------------------------------------------------------
...
- export-control: "Support" (High) — wants legislation to strengthen export controls to keep advanced AI chips from adversaries.
- regulation-philosophy: "Support" (High) — encourage innovation while setting clear guidelines that protect Americans.
- children-safety: "Support" (High) — commonsense safeguards to keep kids safe online, more parental control.
- data-centers: "Opposes data center development" (High) — AI data centers should not disrupt communities or raise energy costs.
- jobs-workforce: "Support" (High) — invest in STEM education; protect workers from invasive AI workplace surveillance.
- deepfakes-fraud: "Oppose" (High) — in the TX legislature, required platforms to create reporting/investigation for explicit deepfakes (i.e. opposes deepfakes).
- intellectual-property: "Support" (High) — AI companies should follow the same copyright laws as everyone else.
- (military-ai, companion-chatbots, AI-preemption: "No mention")
### Tom Cotton (AR, republican, Senate)
- export-control: "Support" (High) — introduced the Chip Security Act (location verification + diversion reporting) and the GAIN AI Act.
- companion-chatbots: "Support" (High) — cosponsored the GUARD Act (age verification for AI chatbots).
- children-safety: "Support" (Medium) — social-media post about holding AI chatbot companies accountable.
- data-centers: "Supports data center development" (High) — DATA Act letting AI data centers bypass federal electricity regulation / build off-grid power.
- AI-preemption: "Oppose" (High) — voted to strike the section preempting state AI law.
- (military-ai, jobs-workforce, deepfakes-fraud, intellectual-property: "No mention")
### David Rouzer (NC, republican, House)
- regulation-philosophy: "Oppose" (Medium) — focus on reducing regulatory barriers so US innovation leads.
- data-centers: "Unclear" (Low) — signed a letter urging silicon carbide in the AI Action Plan to cut AI's energy footprint (engages, no clear dev/reg side).
- jobs-workforce: "Support" (High) — US must support AI literacy and technical skills for the workforce.
- AI-preemption: "Support" (Low) — asked whether lack of federal preemption is a top hurdle to innovation.
- (export-control, military-ai, companion-chatbots, children-safety, deepfakes-fraud, intellectual-property: "No mention")`
...
const PLAYWRIGHT = `For social-media / JS / login-walled pages (WebFetch usually fails on these), read with playwright via Bash: \`playwright-cli open about:blank\`; \`playwright-cli goto "<url>"\`; \`playwright-cli eval "() => document.body.innerText"\`; then run \`playwright-cli close\` when you're done with the page.`

// --- per-member research instructions (this is what makes the 3 detectors DIVERSE) ---
const RESEARCH_BUILTIN = (name) => `# How to research (be exhaustive; don't stop at the campaign site)
Use the WebSearch tool for many distinct queries and WebFetch to read pages. Cover: campaign issues pages; the LEGISLATIVE RECORD (congress.gov sponsor/cosponsor/votes for any current/former legislator); official .gov press; committee/task-force activity (membership counts, often Unclear); news/op-eds/interviews; and a SOCIAL-MEDIA pass (search "${name}" on Facebook/X/Instagram/LinkedIn). ${PLAYWRIGHT}
...
const RESEARCH_STACKED = (name) => `${RESEARCH_BUILTIN(name)}
Also run queries against named AI bills (sponsorship/votes): "TAKE IT DOWN Act", "GUARD Act", "Kids Online Safety Act"/"KOSA", "CHAT Act", "Chip Security Act", "No DeepSeek on Government Devices Act", "DEFIANCE Act", "NO FAKES Act"; plus '"${name}" AI task force' and '"${name}" roll call vote AI'.
...
const RESEARCH_EXA = (name) => `# How to research — use the Exa API (diverse discovery vs plain web search)
Run these via Bash, then READ the most relevant results:
- \`uv run --quiet --with requests python tools/exa.py answer "What are ${name}'s positions on AI policy (export controls, military AI, AI regulation, companion chatbots, kids safety, data centers, jobs, deepfakes, state-vs-federal AI law, IP/copyright)? cite sources"\`  — harvest its CITATION urls as leads (do NOT trust the prose verdict; read the sources).
- \`uv run --quiet --with requests python tools/exa.py search "${name} artificial intelligence" --type keyword --n 25 --highlights "AI policy position"\`
- \`uv run --quiet --with requests python tools/exa.py search "${name} AI" --domains congress.gov --type keyword --n 15 --text 400\`
- \`uv run --quiet --with requests python tools/exa.py search "${name} AI" --domains x.com,twitter.com,facebook.com,instagram.com,linkedin.com --n 15 --highlights "AI"\`
Read promising leads with \`tools/exa.py contents "<url>" --text 4000\` or WebFetch. ${PLAYWRIGHT}`
...
function detectPrompt(c, research) {
  ...
  return `You are a meticulous research analyst building a public, fact-checked tracker (ontheballot.ai) of US congressional candidates' positions on 10 AI-policy topics. Today is the 2026 election cycle.
...
${STANCE_CONVENTIONS}
Be decisive: pick the candidate's dominant direction; a supportive position with a minor caveat is still "Support", not "Mixed". Output {stance, reasoning} via structured output.`
}
...
${TOPIC_RUBRIC}
Verdict: SUPPORTED (genuine + on-topic) / WRONG_TOPIC (real AI content, wrong topic) / NOT_SUPPORTED (not the candidate / no such statement) / UNREACHABLE. NEVER reject on a mere fetch failure (use UNREACHABLE). Return {verdict, reasoning}. (Triage only — this never auto-drops a detection.)`
}
...
const STANCE_SCHEMA = { type:'object', properties:{ stance:{type:'string'}, reasoning:{type:'string'} }, required:['stance','reasoning'] }
...
// then UNION everything. Members miss different cells, so the union is the recall lever.
const detected = await parallel(cands.map(c => async () => {
  ...
  // critic re-check on member A's No-mention topics
  ...
  // union: detected if ANY member (or the critic) detected; keep the highest-confidence hit's text
...
phase('Stance')
// Dedicated 'decisive' stance pass on each DETECTED cell (decoupled from detection).
const stanceJobs = []
for (const r of detected.filter(Boolean))
  for (const t of TOPICS)
    if (r.cell[t].detected)
      stanceJobs.push(async () => {
        const s = await agent(stancePrompt(r.cand, t, r.cell[t].sources), { label:`${r.cand.id}:${t}`, phase:'Stance', schema:STANCE_SCHEMA }).catch(()=>null)
    ...
    for (const t of TOPICS)
      if (r.cell[t].detected)
        vJobs.push(async () => {
          ...
          if (v) r.cell[t].verify = v.verdict   // a triage flag only; never drops the detection
        })
  await parallel(vJobs)
}

// Final result: {cand_id: {topic: {detected, stance, confidence, summary, sources, verify?}}}
const predictions = {}
